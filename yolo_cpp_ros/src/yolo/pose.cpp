...
YoloPose::YoloPose(yolo_onnx_utils::YoloParams params) : Model(params) {}
...
YoloPose::postprocess(const cv::Size &original_image_size,
  ...
  std::vector<yolo_onnx_utils::BoxWithKeypoints> boxes_with_kpts;
  ...
  if (baked_head) {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      ...
      box = yolo_onnx_utils::scale_box(box, original_image_size,
                                       resized_image_size);

      std::vector<yolo_onnx_utils::Keypoint> kpts(static_cast<size_t>(kNumKeypoints));
      ...
      kpts = yolo_onnx_utils::scale_keypoints(
    ...
    boxes_with_kpts.erase(
        std::remove_if(boxes_with_kpts.begin(), boxes_with_kpts.end(),
                       [this](const yolo_onnx_utils::BoxWithKeypoints &b) {
    ...
    const auto indices = yolo_onnx_utils::nms(
        boxes_with_kpts, this->iou_threshold, this->conf_threshold);
    std::vector<yolo_onnx_utils::BoxWithKeypoints> filtered;
  ...
  for (const auto &b : boxes_with_kpts) {
    ...
    detection.bbox = yolo_onnx_utils::convert_to_bounding_box(b);
